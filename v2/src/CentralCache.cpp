...
//   //索引检索，当索引大于等于FREE_LIST_SIZE时，说明申请内存过大应直接向系统申请
...
//         }
//         *reinterpret_cast<void **>(start + (allocBlocks - 1) * size) =
//         nullptr;
//       }
...
//           *reinterpret_cast<void **>(current) = next;
//         }
//         *reinterpret_cast<void **>(start + (totalBlocks - 1) * size) =
//         nullptr;

//         centralFreeList_[index].store(remainStart,
...
//     将原链表头接到归还链表的尾部 centralFreeList_[index].store(
//         start, std::memory_order_release); // 将归还的链表头设为新的链表头
//   } catch (...) {
//     locks_[index].clear(std::memory_order_release);
//     throw;
//   }
